...
import { readRequest, decide } from '../src/state.mjs';
import { DEFAULT_TARGET, PLUGIN_ID, ACTION_NAME } from '../src/defaults.mjs';
...
const input = () => ({ title: '通用确认', summary: 'Review this action', expiresAt: new Date(Date.now() + 60000).toISOString() });
...
test('confirmation goes through Botmux and a bound callback is persisted and patched through the same gateway', async t => {
  ...
  const card = JSON.parse(readFileSync(send[send.indexOf('--card-file') + 1], 'utf8'));
  ...
  decide(directory, { schemaVersion: 1, actionName: ACTION_NAME, larkAppId: DEFAULT_TARGET.larkAppId,
...
test('display cards use Botmux without callback registration; arbitrary callbacks are rejected', async t => {
  ...
  card.body.elements.push({ tag: 'button', behaviors: [{ type: 'callback', value: { action: 'unknown' } }] });
...
test('mismatched recipient and unavailable callback service cannot send a confirmation', async t => {
  let sends = 0;
  const { client, sessionId } = fixture(t, { run: async args => {
...
test('uncertain sends remain send_unknown and are not automatically retried', async t => {
  let sends = 0;
  const { client, directory, sessionId } = fixture(t, { run: async args => {
    if (args[0] === 'history') return { sessionId: args[2], chatId: DEFAULT_TARGET.chatId };
    sends++; throw new Error('transport timeout');
  } });
