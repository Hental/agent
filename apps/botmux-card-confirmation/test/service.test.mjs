...
import { saveRequest, readRequest } from '../src/state.mjs';
import { ACTION_NAME, DEFAULT_TARGET, PLUGIN_ID } from '../src/defaults.mjs';
...
test('loopback callback service requires gateway authentication and returns a terminal generic card', async t => {
  ...
  const request = { ...createRequest({ title: '审批示例', summary: 'Isolated local test', testOnly: true,
    expiresAt: new Date(Date.now() + 60000).toISOString() }, randomUUID()), status: 'pending', messageId: 'om_local_test' };
  saveRequest(directory, request);
  const event = { schemaVersion: 1, actionName: ACTION_NAME, larkAppId: DEFAULT_TARGET.larkAppId,
