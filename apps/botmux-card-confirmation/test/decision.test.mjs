...
import { saveRequest, readRequest, decide } from '../src/state.mjs';
import { DEFAULT_TARGET as TARGET, ACTION_NAME } from '../src/defaults.mjs';
...
function fixture(t) {
  ...
  const request = { ...createRequest({ summary: 'Test', testOnly: true,
    expiresAt: new Date(Date.now() + 60000).toISOString() }, randomUUID()),
    messageId: 'om_test', status: 'pending' };
  saveRequest(dir, request);
  const event = { schemaVersion: 1, actionName: 'card_confirmation_decide', larkAppId: TARGET.larkAppId,
...
test('schema 2 buttons execute callbacks carrying this request and both distinct decisions', t => {
  ...
  const buttons = card.body.elements.find(e => e.tag === 'column_set').columns.map(c => c.elements[0]);
...
test('wrong application, sender, chat, message, nonce and unknown choice cannot approve', t => {
  const { dir, request, event } = fixture(t);
  for (const mutate of [
    e => e.larkAppId = 'wrong', e => e.operator.open_id = 'wrong',
    e => e.context.open_chat_id = 'wrong', e => e.context.open_message_id = 'wrong',
    e => e.action.value.nonce = 'wrong', e => e.action.value.optionId = 'unknown',
  ]) {
    const e = structuredClone(event); mutate(e);
...
test('expired and invalidated requests cannot approve', t => {
  ...
  request.status = 'invalidated'; saveRequest(dir, request);
...
test('business options use stored payload and reject callback-injected business data', t => {
  const { dir, request, event } = fixture(t);
  request.options = [{ id: 'course_1', label: '选择课程', type: 'default', result: 'selected', payload: { scheduleId: 'approved_schedule' } }];
  saveRequest(dir, request);
...
test('requests for another verified target remain isolated', t => {
  const { dir, request, event } = fixture(t);
  request.operatorId = 'ou_another'; saveRequest(dir, request);
...
test('request validation rejects malformed expiry, duplicate options and unsupported results', () => {
  ...
  for (const data of [
    { ...input, expiresAt: 'invalid' }, { ...input, expiresAt: new Date(0).toISOString() },
    { ...input, testOnly: 'false' },
    { ...input, options: [{ id: 'a', label: 'A', result: 'confirmed' }, { id: 'a', label: 'B', result: 'rejected' }] },
    { ...input, options: [{ id: 'a', label: 'A', result: 'execute' }] },
