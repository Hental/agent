...
export function renderCard(request) {
  ...
  const elements = [{ tag: 'markdown', content: request.summary }, {
    tag: 'markdown', content: `**状态：${states[request.status] ?? request.status}**\n确认编号：${request.id}\n有效期：${new Date(request.expiresAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai', hour12: false })}（北京时间）`,
  }];
  ...
  if (request.status === 'pending') {
    for (let i = 0; i < request.options.length; i += 2) {
      elements.push({ tag: 'column_set', flex_mode: 'none', columns: request.options.slice(i, i + 2).map(option => {
        ...
        return { tag: 'column', width: 'weighted', weight: 1,
          elements: [{ tag: 'button', text: { tag: 'plain_text', content: option.label }, type: option.type,
    ...
    elements.push({ tag: 'markdown', content: request.testOnly
  ...
  return {
    ...
    header: { title: { tag: 'plain_text', content: `${request.title}${request.testOnly ? ' · 测试' : ''}` },
