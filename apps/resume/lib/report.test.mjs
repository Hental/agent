...
test('正文和结构化数据的修改共同进入生成结果，重复构建读取最新数据', async () => {
  ...
  try {
    ...
    await writeFile(paths.markdown,original.replace('信息管理与信息系统','UPDATED_SOURCE_CHECK 信息管理与信息系统'));
    await buildReport(paths);
    ...
    d.education.body = 'missing.block'; await writeFile(paths.data,JSON.stringify(d));
    await assert.rejects(()=>renderReport(paths),/Markdown 缺少正文块/);
    d.education.body = 'education.details'; d.projects[0].references[0].source = 'unknown-source';
    await writeFile(paths.data,JSON.stringify(d));
...
test('Markdown 原始 HTML 被转义，非法链接不进入页面', async () => {
  ...
  try {
    ...
    await writeFile(markdown,original.replace('信息管理与信息系统','[link](javascript:alert) 信息管理与信息系统'));
...
test('预览实时读取 MD + JSON，错误可恢复且不暴露任意本地文件', async () => {
  ...
  try {
    ...
    await writeFile(markdown,original.replace('信息管理与信息系统','PREVIEW_MD_UPDATED 信息管理与信息系统'));
