...
```sh
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install --prefix apps/wx-call
```
...
# 只截图，并输出当前桌面逻辑尺寸
node apps/wx-call/lib/control.mjs current

# 点击指定坐标，然后截图；坐标必须来自当前界面
node apps/wx-call/lib/control.mjs after-click 376 213
```
...
最后一步会真正呼叫联系人。群聊位置、窗口位置、分辨率或成员顺序改变后，应重新定位。原始执行脚本已从 `.reports/wechat-call-20260907/control.cjs` 合并迁移至本目录的 `lib/control.mjs`，统一使用 ES Module 入口；历史截图仍保留在 `.reports/wechat-call-20260907/`。后续脚本放在 `apps/wx-call/lib/`，包配置和使用说明放在 `apps/wx-call/`，截图与日志等运行产物放在项目根目录的 `.reports/`。
